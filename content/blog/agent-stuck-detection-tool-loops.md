...
`REPEAT_LIMIT` of 2–3 catches real loops without flagging legitimate re-checks (polling a job status is supposed to repeat — exclude those tools explicitly rather than tuning the threshold around them).
