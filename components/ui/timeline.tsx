...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div className="w-full max-w-[53rem] mx-auto" ref={containerRef}>
      <div ref={ref} className="relative pb-10">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:pt-20 md:gap-8">
            {/* Left side - sticky year/title */}
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-[6rem] md:max-w-[10rem] md:w-[10rem] flex-shrink-0">
              {/* Circle on the timeline */}
              <div className="h-10 absolute left-3 w-10 rounded-full bg-gray-50 dark:bg-neutral-900 flex items-center justify-center">
