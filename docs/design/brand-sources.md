> Asset files live in `public/design/brand/`. Moved out of `public/` so provenance notes are not served publicly.

# Brand originals

`xurrent.png`: official Xurrent webclip, retrieved unchanged from https://cdn.prod.website-files.com/67b20f268356de61cfe3fc70/67fa322c5cf0810424d8cd1d_xurrent%20webclip.png on 2026-09-21 (linked by https://www.xurrent.com).

Mukul's `logo-black.webp`, `logo-white.webp`, and `mukul-original.webp` are the original assets supplied in the portfolio design pack.

Quivly assets retrieved from its official site on 2026-09-19, unchanged:

- `quivly-icon.ico`: https://www.quivly.ai/favicon.ico
- `quivly-wordmark-white.webp`: https://www.quivly.ai/quivly-logo-forest-white.webp

Keep the original proportions and colors. Use the white wordmark on dark backgrounds. The generic Q marks inside generated reference screenshots are illustrative and must not replace the official logo. Standalone Quivly cover artwork contains no logo.
